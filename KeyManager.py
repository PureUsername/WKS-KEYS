...
def add_key(key, pssh, video_url, video_type, video_title):
    key = entry_key.get()
    pssh = entry_pssh.get()
    ...
    keys = key.split('\n')
    for key in keys:
        c.execute("""INSERT INTO keys (key, pssh, video_url, video_type, video_title)
                  VALUES (?, ?, ?, ?, ?)""", (key, pssh, video_url, video_type, video_title))
        conn.commit()
    refresh_listbox()
    update_key_count()
